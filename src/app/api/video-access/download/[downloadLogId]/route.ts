...
import { videoAccessDownloadLogIdSchema } from '@/validations/video-access.validation';
...
function accessError(error: VideoAccessError): NextResponse<ApiResponse<null>> {
  ...
  const messageByCode: Record<string, string> = {
    VIDEO_NOT_FOUND: 'Download not found',
...
export async function GET(
  _req: NextRequest,
  context: { params: Promise<{ downloadLogId: string }> },
) {
  try {
    const session = await requireSession();
    const { downloadLogId } = await context.params;
    const parsed = videoAccessDownloadLogIdSchema.safeParse(downloadLogId);
    if (!parsed.success) {
      return NextResponse.json<ApiResponse<null>>(
        { success: false, data: null, message: 'Invalid download log id' },
        { status: 400 },
      );
    }

    const data = await videoAccessService.getDownloadHistoryVideoAccess(session, parsed.data);
